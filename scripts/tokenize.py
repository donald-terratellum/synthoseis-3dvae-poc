...
def patch_size_arg(values):
    return normalize_patch_size(values)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    build_token.add_argument(
        ...
        nargs='+',
        default=(32,),
        help="Cube patch size: one value for cubic or three values X Y Z",
    )
    ...
    search_volume.add_argument(
        ...
        nargs='+',
        default=(32,),
        help="Search window patch size: one value for cubic or three values X Y Z",
    )
    ...
    search_volume.add_argument(
        ...
        choices=("cosine", "dot"),
        default="cosine",
        help="Similarity metric used to compare latents",
    )
...
def cmd_build_token(args: argparse.Namespace) -> int:
    ...
    runtime.validate()
    patch_shape = normalize_patch_size(args.patch_size)
    ...
    print(
        ...
        f"patch_shape={patch_shape} cube_shape={token_cube.shape} non_zero={non_zero} latent_shape={token_latent.shape} "
        f"latent_mode={args.latent_mode} model={model_path}"
    )
    return 0
...
def cmd_search_volume(args: argparse.Namespace) -> int:
    ...
    token_cube = extract_centered_cube(source_volume, x=cx, y=cy, z=cz, patch_size=patch_shape)
    ...
    print(
        ...
        f"similarity_mode={args.similarity_mode} "
...
def cmd_ui(args: argparse.Namespace) -> int:
    ...
    controller.restore_session_state(auto_load_source=not bool(args.source))
